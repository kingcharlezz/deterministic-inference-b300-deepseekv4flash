...
class ParallelConfig:
    ...
    def _verify_args(self) -> Self:
        ...
        # Batch-invariant mode normally force-disables the custom all-reduce and
        # falls back to (single-channel) NCCL for a deterministic collective.
        # vLLM's custom one-shot/two-shot all-reduce sums in a fixed rank order,
        # which is itself deterministic AND much faster than single-channel NCCL
        # at TP8. VLLM_DETERMINISTIC_ALLOW_CUSTOM_AR=1 keeps custom all-reduce
        # enabled under batch invariance (validate determinism before relying on
        # it for a given platform).
        if envs.VLLM_BATCH_INVARIANT:
            if os.getenv("VLLM_DETERMINISTIC_ALLOW_CUSTOM_AR", "0") != "1":
