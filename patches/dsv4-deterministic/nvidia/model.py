...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        pad_tokens = _deterministic_model_pad_target(num_tokens)
        if residual is None:
            # Run standalone mhc_pre on first layer
            x = _pad_token_dim(x, pad_tokens)
            ...
            x = _pad_token_dim(x, pad_tokens)
            residual = _pad_token_dim(residual, pad_tokens)
            post_mix = _pad_token_dim(post_mix, pad_tokens)
            res_mix = _pad_token_dim(res_mix, pad_tokens)
            assert x is not None
        ...
        post_mix = post_mix[:num_tokens]
        res_mix = res_mix[:num_tokens]
        ...
        if input_ids is not None and input_ids.shape[0] < pad_tokens:
            pad = torch.zeros(
                (pad_tokens - input_ids.shape[0],),
                dtype=input_ids.dtype,
                device=input_ids.device,
            )
            ffn_input_ids = torch.cat((input_ids, pad), dim=0)

        x = self.ffn(x, ffn_input_ids)
        x = x[:num_tokens]
        residual = residual[:num_tokens]
        post_mix = post_mix[:num_tokens]
        res_mix = res_mix[:num_tokens]
