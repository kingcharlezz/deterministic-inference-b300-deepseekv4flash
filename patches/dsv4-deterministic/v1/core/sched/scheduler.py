...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        # Determinism (opt-in via VLLM_DETERMINISTIC_NO_MIX=1): never admit a
        # new prefill in the same step as an in-flight decode. A decode token
        # computed in a mixed prefill+decode step runs at a different (variable)
        # attention/MoE shape than in a pure-decode step, which drifts that
        # token vs an identical request decoded in a pure step. Skipping the
        # waiting phase whenever a decode was already scheduled forces every
        # step to be pure-prefill or pure-decode -> byte-identical decode.
        _no_mix_block = False
        if os.environ.get("VLLM_DETERMINISTIC_NO_MIX", "0") == "1":
            _no_mix_block = any(
