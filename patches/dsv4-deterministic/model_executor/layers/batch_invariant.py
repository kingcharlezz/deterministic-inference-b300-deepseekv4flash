...
def override_envs_for_invariance():
    ...
    # NCCL determinism settings. Single-channel / single-thread all-reduce is a
    # large throughput tax at TP8. VLLM_DETERMINISTIC_FAST_NCCL=1 skips it: the
    # all-reduce becomes multi-channel (faster but its summation order is no
    # longer pinned), trading a small amount of cross-batch determinism for a
    # big collective speedup. Use when the target is "70-90% determinism at
    # high throughput" rather than byte-exact.
    if os.getenv("VLLM_DETERMINISTIC_FAST_NCCL", "0") != "1":
