...
def _deterministic_model_pad_target(num_tokens: int) -> int:
    ...
    prefill_target = int(
        os.getenv("VLLM_DETERMINISTIC_PREFILL_PAD_TOKENS", legacy_target)
    )
    decode_target = int(
        os.getenv("VLLM_DETERMINISTIC_DECODE_PAD_TOKENS", legacy_target)
    )
    target = int(legacy_target)
    try:
        ...
        for metadata in attn_metadata.values():
            ...
            if num_prefill_tokens > 0 or num_decode_tokens > 0:
                if num_prefill_tokens > 0 and num_decode_tokens > 0:
                    # Mixed prefill+decode steps must pad to the SAME fixed M as
                    # pure steps (max, not sum). A larger mixed-only shape (e.g.
                    # 4096) both (a) overflows the MoE workspace locked during
                    # warmup at the pure-step size and (b) gives decode tokens a
                    # different reduction order than pure-decode steps -> drift.
                    target = max(prefill_target, decode_target)
                elif num_prefill_tokens > 0:
                    target = prefill_target
                else:
                    target = decode_target
                break
    return target if target > num_tokens else num_tokens


def _pad_token_dim(x: torch.Tensor | None, target_tokens: int) -> torch.Tensor | None:
    if x is None or x.shape[0] >= target_tokens:
        return x
    pad_shape = (target_tokens - x.shape[0], *x.shape[1:])
    pad = torch.zeros(pad_shape, dtype=x.dtype, device=x.device)
    return torch.cat((x, pad), dim=0)


def _slice_token_dim(x: torch.Tensor | None, num_tokens: int) -> torch.Tensor | None:
    if x is None:
        return None
    return x[:num_tokens]
...
class DeepseekV4Attention(nn.Module, AttentionLayerBase, ABC):
    ...
    def __init__(
        ...
        self.decode_pad_tokens = vllm_config.scheduler_config.max_num_seqs
    ...
    def forward(
        ...
        gemm_tokens = _deterministic_model_pad_target(num_tokens)
        gemm_hidden_states = _pad_token_dim(hidden_states, gemm_tokens)
        assert gemm_hidden_states is not None
        qr_kv, kv_score, indexer_kv_score, indexer_weights = (
            self.attn_gemm_parallel_execute(gemm_hidden_states)
        )
        qr_kv = qr_kv[:num_tokens]
        kv_score = _slice_token_dim(kv_score, num_tokens)
        indexer_kv_score = _slice_token_dim(indexer_kv_score, num_tokens)
        indexer_weights = _slice_token_dim(indexer_weights, num_tokens)
